...
// TODO: Consider passing 'setup' and 'teardown' to QUnit.module.
function setup() {
	$.narayam.setup();
	$.narayam.enable();
}
...
test( "Initialization functions", function () {
	if ( isNarayamMenuShown() ) {
		...
		ok(  $( "#pt-narayam" ).hasClass( "narayam-inactive" ), "After disabling the Narayam menu header has the narayam-inactive class." );
		ok( !$( "#pt-narayam" ).hasClass( "narayam-active" ),   "After disabling the Narayam menu header doesn't have the narayam-active class ." );
		...
		ok( !$( "#pt-narayam" ).hasClass( "narayam-inactive" ), "After enabling the Narayam menu header doesn't have the narayam-inactive class." );
		ok(  $( "#pt-narayam" ).hasClass( "narayam-active" ),   "After enabling the Narayam menu header has the narayam-active class ." );
...
if ( isNarayamMenuShown() ) {
	test( 'Build the menu', function () {
		...
		equal( $( '#pt-narayam' ).length, 1, 'There should be one and only one menu at any time' );
